...
class CastAndCrewWidget extends StatelessWidget {
  const CastAndCrewWidget({super.key,this.isCast=true,this.castVO,this.crewVO});
                          ...
                          return  Padding(
                            ...
                            child: Row(
                              children: [
                                            CircleAvatar(
                                              radius: kSP20x,
                                              backgroundImage: CachedNetworkImageProvider("$kBaseImageUrl${(isCast)? castVO![index].
                                              profile_path??"":crewVO?[index].profile_path??""}")
                                              ),
                                              Padding(
                                                ...
                                                child: Column(
                                                  ...
                                                  children: [
                                                        Text("${(isCast)?castVO![index].character
                                                ??"":crewVO?[index].job??""}",textAlign: TextAlign.left,
                                                        style:const  TextStyle(
                            color: kPrimaryTextColor,
                            fontSize: kCastAndCrewTopTextFS,
                                                        
                                                        ),),
                                                  Text("${(isCast)?castVO![index].
                                              name??"":crewVO?[index].name??""}",
        ...
        return SizedBox(
          ...
          child: Column(
            ...
            children: [
              CircleAvatar(
                radius: kSP25x,
                backgroundImage: CachedNetworkImageProvider("$kBaseImageUrl${productionList?[index].logo_path??""}"),
              ),
              Text(productionList?[index].name??"",style:const  TextStyle(
...
class MoviesSessionViewWidget extends StatelessWidget {
  const MoviesSessionViewWidget({
    super.key, required this.movieSessionViewHeight,this.movieList,
    this.knownForList,
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        scrollDirection: Axis.horizontal,
        itemCount: (isKnownFor)?knownForList!.length??0 :movieList?.length??0,
          itemBuilder: (context,index){
          return GestureDetector(
            onTap: (){
              Navigator.of(context).push(MaterialPageRoute(builder: (context){
                return MovieDetailPage(movieID: (isKnownFor)? knownForList![index].id??0:movieList?[index].id??0);
              }));
            },
            child: Stack(
              children:[
                ...
                child: ClipRRect(
                  ...
                  child:CachedNetworkImage(
                    ...
                    imageUrl:"$kBaseImageUrl${(isKnownFor)? knownForList![index].poster_path??"" :movieList?[index].poster_path??""}",
                    fit: BoxFit.fill,
                  ) ,
                ),
              ),
                     ...
                     children: [
                       Padding(
                         ...
                         child: SizedBox(
                           ...
                           child: Text("${(isKnownFor)? knownForList![index].title??"" :movieList?[index].title??""}",
                           maxLines: 2,
                       ...
                       Row(
                         children: [
                           ...
                           Padding(
                             ...
                             child: Text("${(isKnownFor)? knownForList![index].vote_average.toString()??"" :movieList?[index].vote_average.toString()??""}",style: TextStyle(
                           ...
                           Padding(
                             ...
                             child: Text("${(isKnownFor)? knownForList![index].vote_count.toString()??"" :movieList?[index].vote_count.toString()??""}"+" votes",style: TextStyle(
                               color: kRateAndVoteColor,
                               fontSize: kNPRateandVoteFS
                             ),),
                           )
